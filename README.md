...
2. Создайте `secrets/flibusta_pwd.txt` с паролем PostgreSQL и ограничьте права на чтение. PHP-FPM и оба обработчика используют UID/GID `82:82`: дайте ему запись в `FlibustaSQL/` и `cache/`, чтение архивов и необходимых secrets. При обновлении остановите обработчики и передайте существующий `cache/compilations/` владельцу `82:82`, сохранив закрытые права каталогов и файлов. Ручные команды worker также запускайте от UID 82, например `docker compose exec --user 82:82 php-fpm php /application/tools/app_worker.php`.
