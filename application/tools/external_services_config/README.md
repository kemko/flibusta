...
Оба обработчика работают от `82:82`, как PHP-FPM. Этому UID нужны запись в `cache`, чтение архивов и соответствующих secrets. При обновлении остановите обработчики и передайте существующий `cache/compilations/` владельцу `82:82`; ручные команды worker запускайте с `docker compose exec --user 82:82 flibusta-fpm`.
